...
이러한 기능의 자세한 내용은 <doc:Properties>, <doc:Methods>, <doc:Initialization>, <doc:Extensions>, 과 <doc:Protocols> 을 참고 바랍니다.
...
<doc:ControlFlow> 에서 설명 했듯이 `switch` 구문은 열거형 케이스를 고려할 때 완벽해야 합니다. `.west` 에 대한 `case` 가 생략된다면 `CompassPoint` 케이스에 대해 모든 리스트이 고려되지 않았으므로 이 코드는 컴파일 되지 않습니다. 완전성을 요구하면 열거형 케이스가 실수로 생략되지 않도록 합니다.
...
위 예제에서 사용된 구문은 열거형이 [`CaseIterable`](https://developer.apple.com/documentation/swift/caseiterable) 프로토콜을 준수하는 것으로 표시합니다. 자세한 내용은 <doc:Protocols> 을 참조 바랍니다.
...
<doc:Enumerations#스위치-구문에서-열거형-값-일치-Matching-Enumeration-Values-with-a-Switch-Statement> 에서의 예제와 유사하게 스위치 구문을 이용하여 다른 바코드 타입을 확인할 수 있습니다. 그러나 이번에는 관련값이 스위치 구문의 일부로 추출됩니다. `switch` 케이스의 본문 내에서 사용하기 위해 상수 \(`let` 접두사\) 또는 변수 \(`var` 접두사\)로 각 연관된 값을 추출합니다:
...
<doc:Enumerations#연관된-값-Associated-Values> 에서의 바코드 예제는 어떻게 열거형 케이스에 다른 타입의 연관된 값을 저장한다고 선언 하는지를 보여줍니다. 연관된 값의 대안으로 열거형 케이스는 모두 동일한 타입의 기본값 \(_원시값 \(raw values\)_\)으로 미리 채워질 수 있습니다.
...
여기서 `ASCIIControlCharacter` 라 하는 열거형의 원시값은 `Character` 타입으로 정의되고 일반적인 ASCII 제어 문자 중 일부를 선정합니다. `Character` 값은 <doc:StringsAndCharacters> 에서 설명되어 있습니다.
...
> Note  
> 원시값 초기화는 모든 원시값을 열거형 케이스로 반환할 수 없으므로 초기화가 실패할 수 있습니다. 더 자세한 내용은 <doc:Initialization#실패-가능한-초기화-구문-Failable-Initializers> 을 참고 바랍니다.
